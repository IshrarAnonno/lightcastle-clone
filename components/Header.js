    ...
    <header className="sticky top-0 z-35 bg-slate-950/70 backdrop-blur-md border-b border-slate-900/60 py-4 px-6 md:px-12">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-8 text-xs font-bold tracking-wider text-slate-400">
          <Link 
            href="/about" 
            className={`hover:text-slate-100 transition-colors ${isLinkActive('/about') ? 'text-brand-blue' : ''}`}
          >
            ABOUT
          </Link>
          <Link 
            href="/services" 
            className={`hover:text-slate-100 transition-colors ${isLinkActive('/services') || router.pathname.startsWith('/services/') ? 'text-brand-blue' : ''}`}
          >
            SERVICES
          </Link>
          <Link 
            href="/insights" 
            className={`hover:text-slate-100 transition-colors ${isLinkActive('/insights') || router.pathname.startsWith('/insights/') ? 'text-brand-blue' : ''}`}
          >
            INSIGHTS
          </Link>
          <Link 
            href="/contact" 
            className={`hover:text-slate-100 transition-colors ${isLinkActive('/contact') ? 'text-brand-blue' : ''}`}
          >
            PARTNER WITH US
          </Link>
        </nav>
